...
import kotlinx.android.synthetic.main.activity_nursery_filtered_profile.*
import kotlinx.android.synthetic.main.activity_nursery_profile.*


class NurseryFilteredProfileActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_nursery_filtered_profile)

        //αποδοχή του uid του σταθμού που επέλεξε ο χρήστης από το FilteredListActivity, ώστε να προβληθούν τα στοιχεία αυτού του σταθμού
        val intent = intent
        val nurseryuid = intent.getStringExtra("profileId")
        ...
        photos_button_fprofile_nursery.setOnClickListener {
            val photoIntent = Intent(this@NurseryFilteredProfileActivity, PhotoGalleryForOthersActivity::class.java)
        ...
        femail_icon_nursery.setOnClickListener {

            sendEmail()
        }

        //οδηγεί στην κλήση τηλεφωνικού αριθμού από την συσκευή του χρήστη
        fphone_icon_nursery.setOnClickListener {
            ...
            override fun onDataChange(p0: DataSnapshot) {
                ...
                if (thePhone.trim().isNotEmpty()) {
                    ...
                    if (ContextCompat.checkSelfPermission(
                            this@NurseryFilteredProfileActivity,
    ...
    private fun nurseryInfo(){
        //δημιουργία αναφοράς της βάσης στον ΠΣ που επέλεξε ο χρήστης στο FilteredListActivity
        ...
        nurseryRef.addValueEventListener(object: ValueEventListener {
            ...
            override fun onDataChange(p0: DataSnapshot) {
                ...
                if (p0.exists()){
                    val nursery = p0.getValue<Nursery>(Nursery::class.java)
                    Picasso.get().load(nursery!!.image).placeholder(R.drawable.ic_action_profile).into(profile_image_fprofile_nursery)
                    name_textview_fprofile_nursery?.text = nursery!!.name
                    nomos_spinner_fprofile_nursery?.text = nursery!!.prefecture
                    dimos_spinner_fprofile_nursery?.text = nursery!!.municipality
                    address_textview_fprofile_nursery?.text = nursery!!.address
                    email_textview_fprofile_nursery?.text = nursery!!.email
                    phone_textview_fprofile_nursery?.text = nursery!!.phone
                    activities_textview_fprofile_nursery?.text = nursery!!.activities
                    ages_spinner_fprofile_nursery?.text = nursery!!.ages
                    espa_spinner_fprofile_nursery?.text = nursery!!.espa
                    food_spinner_fprofile_nursery?.text = nursery!!.food
                }
